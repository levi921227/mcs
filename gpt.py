...
class Agent:
    ...
    def declare_r_coin(self, amount):
        if amount > self.r_coin:
            print(f"Error: Agent {self.id} doesn't have enough R-coin.")
            return 0
        return amount
...
class Server:
    ...
    def select_verifier(self, agents, total_r_coin, system_s_coin):
        return select_verifier_simplified(agents, total_r_coin, system_s_coin, blockchain)
...
def select_verifier_simplified(agents, total_r_coin, system_s_coin, blockchain):
    if total_r_coin == 0:
        return None
    for agent in agents:
        declared_r = agent.declare_r_coin(random.randint(0, min(100, int(agent.r_coin))))
        if declared_r > 0:
            agent.r_coin -= declared_r
            agent.s_coin += system_s_coin * (declared_r / total_r_coin)
    random_values = [generate_random_hash(str(agent.s_coin)) for agent in agents]
    return agents[random_values.index(max(random_values))]
...
num_agents, initial_r_coin, system_s_coin = 5, 100, 100
agents = [Agent(i, random.randint(50, initial_r_coin)) for i in range(num_agents)]
server, blockchain = Server(), Blockchain()

for _ in range(10):
    simulate_crowdsensing(blockchain, server, agents, system_s_coin)
    print("\n")
